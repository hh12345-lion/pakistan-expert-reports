...
/** Highest-volume transactional profiles (Appendix B priority boost) */
const HIGH_PRIORITY_PROFILE_SLUGS = new Set([
  "ahmadi-muslims-pakistan",
  "blasphemy-accusations-pakistan",
]);

/** First-class static routes (indexable). Excludes contact, thank-you, privacy, terms, cookie-policy. */
export const APP_STATIC_PATHS: PublicUrlEntry[] = [
  ...
  { path: "/what-is-a-pakistan-expert-report", priority: 0.9, changefreq: "monthly" },
...
function dynamicEntries(): PublicUrlEntry[] {
  return [
    ...asylumProfiles.map((p) => ({
      ...
      priority: HIGH_PRIORITY_PROFILE_SLUGS.has(p.slug) ? 0.94 : 0.92,
